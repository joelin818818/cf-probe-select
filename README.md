...
- 表头（域名 / IP / 国家 / CF 判定 / 延迟 / 状态）均可点击切换排序；
- **国家列**：每个域名取解析出的首个 IP，由浏览器直接查询 ipwho.is（备用 freeipapi.com、ipapi.co）得到归属地，服务端不发起请求；三源均失败则显示 `?`。每个源独立限流，遇到 429 自动退避重试；同一 IP 仅查一次并缓存；ISO alpha-2 代码经内置全量中文映射表转为中文名（约 249 个国家/地区），悬停可见「中文名·代码·英文名」。
