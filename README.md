...
- **两阶段测速**，兼顾速度与精度：
  1. **粗筛**：全量域名各测 **1 轮**，快速得到完整排序（一两分钟出全貌）；
  2. **精测**：仅对粗筛里延迟最低的「精测数量」个域名测 **3 轮**（轮间间隔 2 秒）取平均。

  若 400 个域名全部各测 3 轮，光轮间等待就要 20 多分钟，两阶段能让你尽快看到可用结果。
- **精测数量**（默认 50，范围 1–400）、解析 / 测速并发数（默认 16 / 10，范围 1–32）均可在页面顶部调整，存于 `localStorage`。
- 延迟列展示各轮成绩（数字 = 毫秒，`T` = 超时，`E` = 失败）与平均成绩；鼠标悬停可看该行是「粗筛 1 轮」还是「精测 3 轮」。
- 排序规则：精测结果优先（多轮数据更可信）→ 成功率降序 → 平均延迟升序。
- **筛选与搜索**：可按域名关键字搜索，并可勾选「仅 CF 节点」「仅可达」；筛选只影响表格显示，顶部指标卡始终统计全量。
- **复制当前**：复制当前筛选后可见的域名（附平均延迟），做到所见即所得。
...
所有可调参数集中在 `cf_probe_select.py` **文件头部的「可调参数配置区」**，改那一个地方即可，无需翻找代码其他位置。
